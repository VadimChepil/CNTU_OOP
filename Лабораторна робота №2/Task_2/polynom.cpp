...
void Polynom::divisionIntoGroups(const string& polynom)
{
    ...
    for (size_t i = 0; i < polynom.size(); ++i)
    {
        ...
        if (c == '+' || c == '-')
        {
            ...
            {
                if (currentTerm.find("xy^") != string::npos)
                {
                    xyPowerGroup.push_back(currentTerm);
                }
                else if (currentTerm.find("xy") != string::npos)
                {
                    xyGroup.push_back(currentTerm);
                }
                else if (currentTerm.find("x^") != string::npos)
                {
                    xPowerGroup.push_back(currentTerm);
                }
                else if (currentTerm.find("y^") != string::npos)
                {
                    yPowerGroup.push_back(currentTerm);
                }
                else if (currentTerm.find("x") != string::npos)
                {
                    xGroup.push_back(currentTerm);
                }
                else if (currentTerm.find("y") != string::npos)
                {
                    yGroup.push_back(currentTerm);
                }
                else
                {
                    constantGroup.push_back(currentTerm);
                }
            }
            currentTerm = c;
        }
...
void Polynom::changeSignInGroups() {
    for (auto& term : xGroup) {
        changeSign(term);
    }
    for (auto& term : yGroup) {
        changeSign(term);
    }
    for (auto& term : xPowerGroup) {
        changeSign(term);
    }
    for (auto& term : yPowerGroup) {
        changeSign(term);
    }
    for (auto& term : xyGroup) {
        changeSign(term);
    }
    for (auto& term : xyPowerGroup) {
        changeSign(term);
    }
    for (auto& term : constantGroup) {
        changeSign(term);
    }
}
...
string Polynom::processSimpleGroup(const vector<string>& group, char var, char action) const
{
    ...
    for (const auto& term : group)
    {
        ...
        if (action == '+')
        {
            sum += (sign == '-') ? -coefficient : coefficient;
        }
        else if (action == '-')
        {
            sum += (sign == '-') ? coefficient : -coefficient;
...
string Polynom::processGroupWithPower(const vector<string>& group, char var1, char var2, char action, bool isPower) const
{
    ...
    if (sum != 0)
    {
        if (isPower && var2 != 'q')
        {
            result = (sum < 0) ? to_string(sum) + var1 + var2 + "^2 " : "+" + to_string(sum) + var1 + var2 + "^2 ";
        }
        else if (!isPower && var2 != 'q')
        {
            result = (sum < 0) ? to_string(sum) + var1 + var2 + " " : "+" + to_string(sum) + var1 + var2 + " ";
        }
        else
        {
            result = (sum < 0) ? to_string(sum) + var1 + "^2 " : "+" + to_string(sum) + var1 + "^2 ";
        }
    }
    return result;
}
...
string Polynom::add(const Polynom& other) const
{
    ...
    int sumConstant = 0;
    for (const string& term : resultAdd.constantGroup)
    {
        sumConstant += stoi(term);
    }

    if (sumConstant != 0)
    {
        resultString += (sumConstant < 0) ? to_string(sumConstant) + " " : "+" + to_string(sumConstant) + " ";
    }
...
string Polynom::subtract(const Polynom& other) const
{
    ...
    int sumConstant = 0;
    for (const string& term : resultSubtract.constantGroup)
    {
        sumConstant += stoi(term);
    }
    if (sumConstant != 0)
...
string Polynom::multiply(const Polynom& other) const
{
    ...
    for (const auto& term1 : xGroup) {
        for (const auto& term2 : other.xGroup) {
            result.xPowerGroup.push_back(multiplyTerms(term1, term2, 'x', true));
        }
    }
    for (const auto& term1 : xGroup) {
        for (const auto& term2 : other.constantGroup) {
            result.xGroup.push_back(multiplyTerms(term1, term2, 'x', false));
        }
    }
    for (const auto& term1 : constantGroup) {
        for (const auto& term2 : other.constantGroup) {
            result.constantGroup.push_back(multiplyTerms(term1, term2, ' ', false));
        }
    }
...
string Polynom::multiplyTerms(const string& term1, const string& term2, char var, bool isPower) const
{
    ...
    if (isPower) {
        return to_string(resultCoef) + var + "^2";
    } else if (var == ' ') {
        return to_string(resultCoef);
    } else {
        return to_string(resultCoef) + var;
    }
}
